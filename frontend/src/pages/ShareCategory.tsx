...
import { getCategory, getCategoryMembers, inviteCategoryMember, removeCategoryMember } from '../api/products';
import { AppBar } from '../components/AppBar';
import { useAuthStore } from '../store/authStore';
import type { ListMemberDto } from '../types';

function memberLabel(m: ListMemberDto, currentUserId: string): string {
  const name = m.displayName?.trim() || m.email || m.phone || 'חבר/ה בקטגוריה';
...
export function ShareCategory() {
  ...
  const { data: members = [], isLoading } = useQuery({
    queryKey: ['categoryMembers', categoryId],
    queryFn: () => getCategoryMembers(categoryId!),
    enabled: !!categoryId,
  });

  const inviteMutation = useMutation({
    mutationFn: (body: { email?: string; phone?: string }) => inviteCategoryMember(categoryId!, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['categoryMembers', categoryId] });
      setInviteEmail('');
      setInvitePhone('');
      setInviteError(null);
    },
  ...
  const removeMutation = useMutation({
    mutationFn: (memberUserId: string) => removeCategoryMember(categoryId!, memberUserId),
    onSuccess: (_data, memberUserId) => {
      queryClient.invalidateQueries({ queryKey: ['categoryMembers', categoryId] });
      queryClient.invalidateQueries({ queryKey: ['categories'] });
      queryClient.invalidateQueries({ queryKey: ['products'] });
      if (memberUserId === currentUserId) navigate('/categories');
    },
    onError: (err: Error) => {
  ...
  const isOwner = category?.ownerId === currentUserId;
  ...
  return (
    <>
      <AppBar
        title={category ? `שיתוף: ${category.nameHe}` : 'שיתוף קטגוריה'}
        backTo={`/categories`}
      />
      <main style={{ padding: 16 }}>
        <p style={{ margin: '0 0 20px', color: '#555', fontSize: 15 }}>
          חברים בקטגוריה יראו אותה ואת מוצריה בבנק המוצרים ויוכלו לערוך. הוסף משתמשים לפי אימייל או מספר טלפון.
        </p>
        ...
        {isLoading ? (
          ...
          <>
            <section style={{ marginBottom: 24 }}>
              <h2 style={{ margin: '0 0 12px', fontSize: 18, fontWeight: 600 }}>חברים בקטגוריה</h2>
                  ...
                  >
                    <div>
                      ...
                      <span style={{ marginRight: 8, fontSize: 13, color: '#666' }}>
                        {m.role === 'owner' ? 'בעל/ת הקטגוריה' : 'עורך/ת'}
                      </span>
                    </div>
                    {m.role !== 'owner' && (
                      <button
                      ...
                      >
                        {m.userId === currentUserId ? 'עזוב קטגוריה' : 'הסר'}
                      </button>
                    )}
                  </li>
                ))}
