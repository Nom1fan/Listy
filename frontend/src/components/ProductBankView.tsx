...
export interface ProductBankViewProps {
  listId: string;
  /** Called after a product is successfully added to the list. Useful for parent toasts. */
  onItemAdded?: (productName: string) => void;
}

/**
 * Browse the workspace product bank scoped to a list's attached categories.
 * Used both as a standalone page and inside the "add from categories" bottom-sheet
 * on the list detail screen.
 */
export function ProductBankView({ listId, onItemAdded }: ProductBankViewProps) {
  ...
  return (
    <div data-testid="product-bank-view">
      {toast && (
        <div
